...
if not args.type:
	...
	grab_prompt_dir_cmd = f"echo {delimiter}; (Get-Location).path" if payload_type in ['cmd-curl'] else "echo `r;pwd"
			...
			if Hoaxshell.prompt_ready:
				...
				if user_input_lower in ['clear']:
					...
					if Hoaxshell.execution_verified and not Hoaxshell.command_pool:
						...
						full_command = f"{user_input}; echo {delimiter}; (Get-Location).path " if payload_type in ['cmd-curl'] else user_input + ";pwd"
